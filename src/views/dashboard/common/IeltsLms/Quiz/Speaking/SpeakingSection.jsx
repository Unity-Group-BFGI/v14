...
const IeltsLmsEditQuizSpeakingSections = () => {
    const HEADING = 'List of Speaking Sections';
    const BTN_TEXT = 'Add Section';
    const NO_ITEMS_TEXT = "0 Sections";
    ...
    const getQuestions = (sid)  => {
        ...
        dispatch(update_menu({
            CURRENT_DYNAMIC_PARAM: 'IELTS-LMS-EDIT-QUIZ-SPEAKING-QUESTIONS'
        }));
    };
    ...
    useEffect(() => {
        ...
        if ( 
            ...
            toast.warning("Failed to locate Speaking sections endpoints", {
                position: "top-right",
                autoClose: 10000,
                hideProgressBar: false,
                closeOnClick: true,
...
const SectionItem = ({ section, index, handleRefresh, getQuestions, showEditSectionModal , length }) => {
    const POST_TYPE                                     = 'speaking-section';
    const QUIZ_CATEGORY                                 = 'speaking';
    const { id }                                        = useParams();
    const { ENDPOINT }                                  = ENDPOINTS;
    ...
    const confirmRestore = (sid) => {
        if( IELTS_LMS_EDIT_QUIZ_EDIT_SECTION_ROUTE && IELTS_LMS_EDIT_QUIZ_EDIT_SECTION_ROUTE.endpoint ) {
            ...
            toast.error("Edit section endpoint not found", {
                position: "top-right",
                autoClose: 10000,
                hideProgressBar: false,
                closeOnClick: true,
    ...
    const confirmRemove = (sid) => {
        if( IELTS_LMS_EDIT_QUIZ_REMOVE_SECTION_ROUTE && IELTS_LMS_EDIT_QUIZ_REMOVE_SECTION_ROUTE.endpoint ) {
            ...
            toast.error("Remove section endpoint not found", {
                position: "top-right",
                autoClose: 10000,
                hideProgressBar: false,
                closeOnClick: true,
...
const AddEditSectionModal = ({ reload, showSectionModal, handleCloseAddEditSection, handleRefresh, type, sid, cancelTokenSource }) => {
        ...
        if( 
            ...
            if(type == "new"){
                axios.post(`${ENDPOINT}${IELTS_LMS_EDIT_QUIZ_ADD_SECTION_ROUTE.endpoint}/${id}`, {
                    ...
                    if (response.data.success) {
                        if (response.data.success.has_json) {
                            handleCloseAddEditSection(true);
                        }
                    }
    ...
    useEffect(() => {
        if( type === "new" ) {
            ...
            setModalTitle('Add Speaking section');
            setBtnText("Add Section");
            setTitle("");
            setHtml("");
            setOrder(1);
            setHtmlEditor(<LoadEditorLight html={"Add Section content"} setHtml={setHtml} />);
            ...
            setModalTitle('Edit Speaking section');
            setBtnText("Edit Section");
            setOnce(1);
            loadSection();
        }
...
export default IeltsLmsEditQuizSpeakingSections;