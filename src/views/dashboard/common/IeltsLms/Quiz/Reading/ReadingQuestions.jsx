...
import { READING_QUESTION_CATEGORIES } from "../../../../../../includes/constants/ieltslms";
...
const IeltsLmsEditQuizReadingQuestions = () => {
    ...
    useEffect(() => {
        ...
        if (
            ...
            toast.warning("Failed to locate Reading Questions endpoints", {
                position: "top-right",
                autoClose: 10000,
                hideProgressBar: false,
                closeOnClick: true,
                            ...
                            <div className="card-body">
                                ...
                                <div className="text-center pt-10 mb-20">
                                    ...
                                    <button type='button' className="btn btn-primary btn-thin" style={{ marginRight: "15px" }} onClick={() => dispatch(update_menu({
                                        CURRENT_DYNAMIC_PARAM: 'IELTS-LMS-EDIT-QUIZ-READING-PASSAGES'
                                    }))}>
...
const QuestionsList = ({ sid }) => {
    ...
    useEffect(() => {
        ...
        if (
            ...
            toast.warning("Failed to locate Reading passages endpoints", {
                position: "top-right",
                autoClose: 10000,
                hideProgressBar: false,
                closeOnClick: true,
...
const AddEditQuestionModal = ({ type, qid, sid, handleClose, addEditQuestionModal }) => {
    ...
    const POST_TYPE = 'reading-question';
    const { ENDPOINT } = ENDPOINTS;
    const dispatch = useDispatch();
    const { id } = useParams();
    const { IELTS_LMS_EDIT_QUIZ_GET_QUESTION_ROUTE } = useSelector(state => state.constants);
    ...
    const { IELTS_LMS_EDIT_QUIZ_QUESTION } = useSelector(state => state.ieltslms);
    const [HEADING, setHeading] = useState('Add Reading Question');
    ...
    useEffect(() => {
        if (sid) {
            if (type == "new") {
                setHeading('Add Reading Question');
                ...
                setHeading('Edit Reading Question');
                loadQuestion();
            }
        } else {
                        ...
                        <div className="input-group input-group-solid mb-5">
                            {!showCategoryForm && <>
                                <select disabled={categoryStates.LOADING || processing} onChange={(event) => dispatch(update_ieltslms_editQuiz_question_update_question_main({
                                    ...
                                    {READING_QUESTION_CATEGORIES && READING_QUESTION_CATEGORIES.length > 0 && <>
                                        <optgroup label="--Default Reading categories--">
                                            {READING_QUESTION_CATEGORIES.map((o,i) => {
                                                const { value, label } = o;
                                    ...
                                    {categories && categories.length > 0 && <>
                                        <optgroup label="--Custom Reading categories--">
...
export default IeltsLmsEditQuizReadingQuestions;