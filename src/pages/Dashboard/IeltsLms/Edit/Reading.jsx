...
import { QuestionMainMatchingHeadings } from './ReadingQuestions/MatchingHeadings';
import { QuestionMainMatchingInformation } from './ReadingQuestions/MatchingInformation';
import { QuestionMainTrueFalseNotgiven } from './ReadingQuestions/TrueFalseNotGiven';
import { QuestionMainYesNoNotgiven } from './ReadingQuestions/YesNoNotGiven';
import { QuestionMainSummaryFormCompletion } from './ReadingQuestions/SummaryFormCompletion';
import { QuestionMainSentenceCompletion } from './ReadingQuestions/SentenceCompletion';
import { QuestionMainPlanMapDiagramLabelling } from './ReadingQuestions/PlanMapDiagramLabelling';


const IeltsLmsEditQuizReadingPassages = () => {
    ...
    useEffect(() => {
        ...
        if(
            ...
            if(
                ...
                if(PARAM3 === "ielts-lms-edit-quiz-reading-passages"){
                    
                    if(PARAM4 === "ielts-lms-edit-quiz-reading-passages-list"){
                        dispatch(update_dashboard({
                            DASHBOARD_ROUTE_LOADING: false,
                        }));
                        setContext(<IeltsLmsEditQuizReadingPassagesList />);
                        return;
                    } else {
                        dispatch(update_route({
                            PARAM4: 'ielts-lms-edit-quiz-reading-passages-list'
                        }));
                    }
                }
            } else {
...
// passages list
const IeltsLmsEditQuizReadingPassagesList = () => {
    const { id }                                                = useParams();
    const { endpoint }                                          = constants;
    const dispatch                                              = useDispatch();
    const [loading,setLoading]                                  = useState(true);
        ...
        <div className="card mb-0 bg-transparent" style={{boxShadow: '0 0 0 '}}>
            ...
            <div className="card-header border-0 cursor-pointer bg-transparent">
                <div className="card-title m-0">
                    <h3 className="fw-bold m-0">List of Reading Passages</h3>
                </div>
    ...
    return (<>
        ...
        >
            <div className="modal-header p-3">
                <h4 className='p-0 m-0'>Edit Reading Passage</h4>
...
// questions list
const IeltsLmsEditQuizReadingQuestions = () => {
    ...
    useEffect(() => {
        if(
            ...
            if(
                ...
                if(PARAM3 === "ielts-lms-edit-quiz-reading-questions"){
                    dispatch(update_dashboard({
                        DASHBOARD_ROUTE_LOADING: false,
                    }));
                    setLoading(false); 
                                            ...
                                            <div className="d-flex flex-stack">
                                                ...
                                                <div className="d-flex flex-column justify-content-evenly">
                                                    <div className="d-flex flex-row justify-content-evenly mb-2">
                                                        ...
                                                        {status == "trashed" && <>
                                                            <button disabled={addQuestionModal} onClick={() => {}} className="btn btn-sm btn-light-primary btn-icon btn-link mr-2" type='button'>
                                                                <i className="fa fa-refresh"></i>
                                                            </button>
                                                            <button disabled={addQuestionModal} className="btn btn-sm btn-light-danger" type='button' style={{padding: '4px 10px !important'}}>
                                                                <i className="fa fa-trash"></i> Delete Permanent
                                                            </button>
                                                        </>}
                                                        {(status == "published" || status == "drafted") && <>
                                                            <button disabled={addQuestionModal} className="btn btn-sm btn-light-primary btn-icon btn-link" style={{marginRight: '10px'}} type='button' onClick={() => editQuestion(_id)}>
                                                                <i className="fa fa-pencil"></i>
                                                            </button>
                                                            <button disabled={addQuestionModal} onClick={() => {}} className="btn btn-sm btn-light-danger btn-icon btn-link mr-2" type='button'>
    ...
    return (<>
        ...
        >
            ...
            <div className="modal-header p-4">
                <h6 className='modal-title'>Add Reading Question</h6>
...
const QuestionExplanation = () => {
    const dispatch                                          = useDispatch();
    const { IELTS_LMS_QUESTION_TYPE }                       = useSelector( state => state.ieltsLms );
    const { IELTS_LMS_QUESTION_MATCHING_HEADINGS_READING }          = useSelector( state => state.ieltsLms );
    ...
    const { IELTS_LMS_QUESTION_TRUE_FALSE_NOTGIVEN_READING }        = useSelector( state => state.ieltsLms );
    const { IELTS_LMS_QUESTION_SUMMARY_FORM_COMPLETION_READING }    = useSelector( state => state.ieltsLms );
    ...
    const updateExplanationHTML = (targetKey, newHTML) => {
        if(IELTS_LMS_QUESTION_TYPE === "matching-headings"){
            dispatch(update_question_matching_headings_explanation({
                index: targetKey,
                html: newHTML
            }));
        } else if(IELTS_LMS_QUESTION_TYPE === "matching-information"){
            dispatch(update_question_matching_information_explanation({
                index: targetKey,
                html: newHTML
            }));
        } else if(IELTS_LMS_QUESTION_TYPE === "true-false-notgiven"){
            dispatch(update_question_true_false_notgiven_explanation({
                index: targetKey,
                html: newHTML
            }));
        } else if(IELTS_LMS_QUESTION_TYPE === "yes-no-notgiven"){
            dispatch(update_question_yes_no_notgiven_explanation({
                index: targetKey,
                html: newHTML
            }));
        } else if(IELTS_LMS_QUESTION_TYPE === "summary-form-completion"){
            dispatch(update_question_summary_form_completion_explanation({
                index: targetKey,
                html: newHTML
            }));
        } else if(IELTS_LMS_QUESTION_TYPE === "sentence-completion"){
            dispatch(update_question_sentence_completion_explanation({
                index: targetKey,
                html: newHTML
            }));
        } else if(IELTS_LMS_QUESTION_TYPE === "plan-map-diagram-labelling"){
            dispatch(update_question_plan_map_diagram_labelling_explanation({
                index: targetKey,
                html: newHTML
            }));
        }
    ...
    return (<div style={{background: '#d8f3fb'}}>
        <div className="step-1 steps tab-step__padding pb-10 tiny-css-0">
            <div className="step__controller row">
                ...
                {IELTS_LMS_QUESTION_TYPE === "matching-headings" && <>
                    {IELTS_LMS_QUESTION_MATCHING_HEADINGS_READING.EXPLANATIONS && IELTS_LMS_QUESTION_MATCHING_HEADINGS_READING.EXPLANATIONS.length > 0 && <>
                    {IELTS_LMS_QUESTION_MATCHING_HEADINGS_READING.EXPLANATIONS.map((explaination,index) => {
                ...
                {IELTS_LMS_QUESTION_TYPE === "matching-information" && <>
                    {IELTS_LMS_QUESTION_MATCHING_INFORMATION_READING.EXPLANATIONS && IELTS_LMS_QUESTION_MATCHING_INFORMATION_READING.EXPLANATIONS.length > 0 && <>
                    {IELTS_LMS_QUESTION_MATCHING_INFORMATION_READING.EXPLANATIONS.map((explaination,index) => {
                        const { HTML } = explaination;
                        return (<div key={index} className="col-md-12 mb-4">
                            <div className="card shadow-sm">
                                <div className="card-header collapsible cursor-pointer rotate d-flex align-items-center" style={{minHeight: '40px'}}>
                ...
                {IELTS_LMS_QUESTION_TYPE === "true-false-notgiven" && <>
                    {IELTS_LMS_QUESTION_TRUE_FALSE_NOTGIVEN_READING.EXPLANATIONS && IELTS_LMS_QUESTION_TRUE_FALSE_NOTGIVEN_READING.EXPLANATIONS.length > 0 && <>
                    {IELTS_LMS_QUESTION_TRUE_FALSE_NOTGIVEN_READING.EXPLANATIONS.map((explaination,index) => {
                        const { HTML } = explaination;
                        return (<div key={index} className="col-md-12 mb-4">
                            <div className="card shadow-sm">
                                <div className="card-header collapsible cursor-pointer rotate d-flex align-items-center" style={{minHeight: '40px'}}>
                ...
                {IELTS_LMS_QUESTION_TYPE === "yes-no-notgiven" && <>
                    {IELTS_LMS_QUESTION_YES_NO_NOTGIVEN_READING.EXPLANATIONS && IELTS_LMS_QUESTION_YES_NO_NOTGIVEN_READING.EXPLANATIONS.length > 0 && <>
                    {IELTS_LMS_QUESTION_YES_NO_NOTGIVEN_READING.EXPLANATIONS.map((explaination,index) => {
                        const { HTML } = explaination;
                        return (<div key={index} className="col-md-12 mb-4">
                            <div className="card shadow-sm">
                                <div className="card-header collapsible cursor-pointer rotate d-flex align-items-center" style={{minHeight: '40px'}}>
                ...
                {IELTS_LMS_QUESTION_TYPE === "summary-form-completion" && <>
                    {IELTS_LMS_QUESTION_SUMMARY_FORM_COMPLETION_READING.EXPLANATIONS && IELTS_LMS_QUESTION_SUMMARY_FORM_COMPLETION_READING.EXPLANATIONS.length > 0 && <>
                    {IELTS_LMS_QUESTION_SUMMARY_FORM_COMPLETION_READING.EXPLANATIONS.map((explaination,index) => {
                        const { HTML } = explaination;
                        return (<div key={index} className="col-md-12 mb-4">
                            <div className="card shadow-sm">
                                <div className="card-header collapsible cursor-pointer rotate d-flex align-items-center" style={{minHeight: '40px'}}>
                ...
                {IELTS_LMS_QUESTION_TYPE === "sentence-completion" && <>
                    {IELTS_LMS_QUESTION_SENTENCE_COMPLETION_READING.EXPLANATIONS && IELTS_LMS_QUESTION_SENTENCE_COMPLETION_READING.EXPLANATIONS.length > 0 && <>
                    {IELTS_LMS_QUESTION_SENTENCE_COMPLETION_READING.EXPLANATIONS.map((explaination,index) => {
                        const { HTML } = explaination;
                        return (<div key={index} className="col-md-12 mb-4">
                            <div className="card shadow-sm">
                                <div className="card-header collapsible cursor-pointer rotate d-flex align-items-center" style={{minHeight: '40px'}}>
                ...
                {IELTS_LMS_QUESTION_TYPE === "plan-map-diagram-labelling" && <>
                    {IELTS_LMS_QUESTION_PLAN_MAP_DIAGRAM_LABELLING_READING.EXPLANATIONS && IELTS_LMS_QUESTION_PLAN_MAP_DIAGRAM_LABELLING_READING.EXPLANATIONS.length > 0 && <>
                    {IELTS_LMS_QUESTION_PLAN_MAP_DIAGRAM_LABELLING_READING.EXPLANATIONS.map((explaination,index) => {
                        const { HTML } = explaination;
                        return (<div key={index} className="col-md-12 mb-4">
                            <div className="card shadow-sm">
                                <div className="card-header collapsible cursor-pointer rotate d-flex align-items-center" style={{minHeight: '40px'}}>
...
const QuestionBasic = () => {
    ...
    const selectedOptionQuestionLayout = IELTS_LMS_QUESTION_SUMMARY_FORM_COMPLETION_READING.QUESTION_SETTINGS.QUESTION_LAYOUTS.find((layout) => layout.value === IELTS_LMS_QUESTION_SUMMARY_FORM_COMPLETION_READING.QUESTION_SETTINGS.QUESTION_LAYOUT);
    const selectedOptionForSentenceCompletion = IELTS_LMS_QUESTION_SENTENCE_COMPLETION_READING.QUESTION_SETTINGS.QUESTION_LAYOUTS.find((layout) => layout.value === IELTS_LMS_QUESTION_SENTENCE_COMPLETION_READING.QUESTION_SETTINGS.QUESTION_LAYOUT);
    ...
    const handleQuestionLayoutChange = (selectedOptionQuestionLayout) => {
        if(IELTS_LMS_QUESTION_TYPE === "summary-form-completion"){
            const selectedValue = selectedOptionQuestionLayout.value; // Get the selected option's value
            dispatch(update_question_summary_form_completion({
                QUESTION_SETTINGS: {
                    ...IELTS_LMS_QUESTION_SUMMARY_FORM_COMPLETION_READING.QUESTION_SETTINGS,
            ...
            dispatch(update_question_sentence_completion({
                QUESTION_SETTINGS: {
                    ...IELTS_LMS_QUESTION_SENTENCE_COMPLETION_READING.QUESTION_SETTINGS,
                    QUESTION_LAYOUT: selectedValue
                }
            }));
        }
    ...
    const changeNumberOfQuestions = (event) => {
        ...
        if(IELTS_LMS_QUESTION_TYPE === "matching-headings"){
            ...
            for (let i = 0; i < Math.min(num, IELTS_LMS_QUESTION_MATCHING_HEADINGS_READING.EXPLANATIONS.length); i++) {
                newExplanations[i] = { ...IELTS_LMS_QUESTION_MATCHING_HEADINGS_READING.EXPLANATIONS[i] };
            }
        
            dispatch(update_question_matching_headings({
                QUESTION_SETTINGS: {
                    ...IELTS_LMS_QUESTION_MATCHING_HEADINGS_READING.QUESTION_SETTINGS,
            ...
            // Preserve the content from the existing explanations
            for (let i = 0; i < Math.min(num, IELTS_LMS_QUESTION_MATCHING_INFORMATION_READING.EXPLANATIONS.length); i++) {
                newExplanations[i] = { ...IELTS_LMS_QUESTION_MATCHING_INFORMATION_READING.EXPLANATIONS[i] };
            }
        
            dispatch(update_question_matching_information({
                QUESTION_SETTINGS: {
                    ...IELTS_LMS_QUESTION_MATCHING_INFORMATION_READING.QUESTION_SETTINGS,
                    NUMBER_OF_QUESTIONS: num,
                },
                EXPLANATIONS: newExplanations,
            }));
            ...
            // Preserve the content from the existing explanations
            for (let i = 0; i < Math.min(num, IELTS_LMS_QUESTION_TRUE_FALSE_NOTGIVEN_READING.EXPLANATIONS.length); i++) {
                newExplanations[i] = { ...IELTS_LMS_QUESTION_TRUE_FALSE_NOTGIVEN_READING.EXPLANATIONS[i] };
            }
        
            dispatch(update_question_true_false_notgiven({
                QUESTION_SETTINGS: {
                    ...IELTS_LMS_QUESTION_TRUE_FALSE_NOTGIVEN_READING.QUESTION_SETTINGS,
                    NUMBER_OF_QUESTIONS: num,
                },
                EXPLANATIONS: newExplanations
            }));
            ...
            // Preserve the content from the existing explanations
            for (let i = 0; i < Math.min(num, IELTS_LMS_QUESTION_YES_NO_NOTGIVEN_READING.EXPLANATIONS.length); i++) {
                newExplanations[i] = { ...IELTS_LMS_QUESTION_YES_NO_NOTGIVEN_READING.EXPLANATIONS[i] };
            }
        
            dispatch(update_question_yes_no_notgiven({
                QUESTION_SETTINGS: {
                    ...IELTS_LMS_QUESTION_YES_NO_NOTGIVEN_READING.QUESTION_SETTINGS,
                    NUMBER_OF_QUESTIONS: num,
                },
                EXPLANATIONS: newExplanations
            }));
            ...
            for (let i = 0; i < Math.min(num, IELTS_LMS_QUESTION_SUMMARY_FORM_COMPLETION_READING.QUESTIONS.length); i++) {
                newQuestions[i] = { ...IELTS_LMS_QUESTION_SUMMARY_FORM_COMPLETION_READING.QUESTIONS[i] };
            }

            // Preserve the content from the existing explanations
            for (let i = 0; i < Math.min(num, IELTS_LMS_QUESTION_SUMMARY_FORM_COMPLETION_READING.EXPLANATIONS.length); i++) {
                newExplanations[i] = { ...IELTS_LMS_QUESTION_SUMMARY_FORM_COMPLETION_READING.EXPLANATIONS[i] };
            }
            ...
            // Preserve the content from the existing questions
            for (let i = 0; i < Math.min(num, IELTS_LMS_QUESTION_SENTENCE_COMPLETION_READING.QUESTIONS.length); i++) {
                newQuestions[i] = { ...IELTS_LMS_QUESTION_SENTENCE_COMPLETION_READING.QUESTIONS[i] };
            }

            // Preserve the content from the existing explanations
            for (let i = 0; i < Math.min(num, IELTS_LMS_QUESTION_SENTENCE_COMPLETION_READING.EXPLANATIONS.length); i++) {
                newExplanations[i] = { ...IELTS_LMS_QUESTION_SENTENCE_COMPLETION_READING.EXPLANATIONS[i] };
            }
            ...
            // Preserve the content from the existing explanations
            for (let i = 0; i < Math.min(num, IELTS_LMS_QUESTION_PLAN_MAP_DIAGRAM_LABELLING_READING.EXPLANATIONS.length); i++) {
                newExplanations[i] = { ...IELTS_LMS_QUESTION_PLAN_MAP_DIAGRAM_LABELLING_READING.EXPLANATIONS[i] };
            }
        
            dispatch(update_question_plan_map_diagram_labelling({
                QUESTION_SETTINGS: {
                    ...IELTS_LMS_QUESTION_PLAN_MAP_DIAGRAM_LABELLING_READING.QUESTION_SETTINGS,
                    NUMBER_OF_QUESTIONS: num,
                },
                EXPLANATIONS: newExplanations
            }));
        ...
        <div className="step-1 steps tab-step__padding mb-10">
            <div className="step__controller">
                ...
                <Select
                    ...
                    options={IELTS_LMS_QUESTION_TYPES.filter(type => type.category === "reading")}
                ...
                <div className="row mb-4"> 
                    ...
                    <div className="col-sm-12 col-xs-12 mb-4">
                        ...
                        <input 
                            ...
                            value={IELTS_LMS_QUESTION_MATCHING_HEADINGS_READING.QUESTION_SETTINGS.NUMBER_OF_QUESTIONS}
                ...
                <div className="row mb-4"> 
                    ...
                    <div className="col-sm-12 col-xs-12 mb-4">
                        ...
                        <input 
                            ...
                            id="number-of-questions" 
                            className="form-input form-control" 
                            value={IELTS_LMS_QUESTION_MATCHING_INFORMATION_READING.QUESTION_SETTINGS.NUMBER_OF_QUESTIONS}
                            onChange={changeNumberOfQuestions}
                        />    
                    </div>
                ...
                <div className="row mb-4"> 
                    ...
                    <div className="col-sm-12 col-xs-12 mb-4">
                        ...
                        <input 
                            ...
                            id="number-of-questions" 
                            className="form-input form-control" 
                            value={IELTS_LMS_QUESTION_TRUE_FALSE_NOTGIVEN_READING.QUESTION_SETTINGS.NUMBER_OF_QUESTIONS}
                            onChange={changeNumberOfQuestions}
                        />    
                    </div>
                ...
                <div className="row mb-4"> 
                    ...
                    <div className="col-sm-12 col-xs-12 mb-4">
                        ...
                        <input 
                            ...
                            id="number-of-questions" 
                            className="form-input form-control" 
                            value={IELTS_LMS_QUESTION_YES_NO_NOTGIVEN_READING.QUESTION_SETTINGS.NUMBER_OF_QUESTIONS}
                            onChange={changeNumberOfQuestions}
                        />    
                    </div>
                ...
                <div className="row mb-4"> 
                    ...
                    <div className="col-sm-12 col-xs-12 mb-4 col-md-6 mb-4">
                        ...
                        <input 
                            ...
                            id="number-of-questions" 
                            className="form-input form-control" 
                            value={IELTS_LMS_QUESTION_SUMMARY_FORM_COMPLETION_READING.QUESTION_SETTINGS.NUMBER_OF_QUESTIONS}
                            onChange={changeNumberOfQuestions}
                        />    
                    </div>
                    ...
                    <div className="mb-4 col-sm-12 col-xs-12 col-md-6 mb-4">
                        ...
                        <Select
                            ...
                            options={IELTS_LMS_QUESTION_SUMMARY_FORM_COMPLETION_READING.QUESTION_SETTINGS.QUESTION_LAYOUTS}
                ...
                <div className="row mb-4"> 
                    ...
                    <div className="col-sm-12 col-xs-12 mb-4 col-md-6 mb-4">
                        ...
                        <input 
                            ...
                            id="number-of-questions" 
                            className="form-input form-control" 
                            value={IELTS_LMS_QUESTION_SENTENCE_COMPLETION_READING.QUESTION_SETTINGS.NUMBER_OF_QUESTIONS}
                            onChange={changeNumberOfQuestions}
                        />    
                    </div>
                    ...
                    <div className="mb-4 col-sm-12 col-xs-12 col-md-6 mb-4">
                        ...
                        <Select
                            ...
                            options={IELTS_LMS_QUESTION_SENTENCE_COMPLETION_READING.QUESTION_SETTINGS.QUESTION_LAYOUTS}
                            onChange={handleQuestionLayoutChange}
                            styles={{
                                SingleValueLayout: (base) => ({
                                    ...base,
                ...
                <div className="row mb-4"> 
                    ...
                    <div className="col-sm-12 col-xs-12 mb-4">
                        ...
                        <input 
                            ...
                            id="number-of-questions" 
                            className="form-input form-control" 
                            value={IELTS_LMS_QUESTION_PLAN_MAP_DIAGRAM_LABELLING_READING.QUESTION_SETTINGS.NUMBER_OF_QUESTIONS}
                            onChange={changeNumberOfQuestions}
                        />    
                    </div>
...
const QuestionMain = () => {
    const { IELTS_LMS_QUESTION_TYPE }            = useSelector( state => state.ieltsLms );
    return (<>
        {IELTS_LMS_QUESTION_TYPE === "matching-headings" && <QuestionMainMatchingHeadings />}
        {IELTS_LMS_QUESTION_TYPE === "matching-information" && <QuestionMainMatchingInformation />}
        {IELTS_LMS_QUESTION_TYPE === "true-false-notgiven" && <QuestionMainTrueFalseNotgiven />}
        {IELTS_LMS_QUESTION_TYPE === "yes-no-notgiven" && <QuestionMainYesNoNotgiven />}
...
const QuestionPreview = () => {
    ...
    const createExplanations = (html, answerOptions, numberOfQuestions, numberOfOptions) => {
        let explanations = [];
        if(IELTS_LMS_QUESTION_TYPE === "matching-headings"){
            explanations = IELTS_LMS_QUESTION_MATCHING_HEADINGS_READING.EXPLANATIONS;
        } else if(IELTS_LMS_QUESTION_TYPE === "matching-information"){
            explanations = IELTS_LMS_QUESTION_MATCHING_INFORMATION_READING.EXPLANATIONS;
        } else if(IELTS_LMS_QUESTION_TYPE === "true-false-notgiven"){
            explanations = IELTS_LMS_QUESTION_TRUE_FALSE_NOTGIVEN_READING.EXPLANATIONS;
        }
    ...
    useEffect(() => {
        if(IELTS_LMS_QUESTION_TYPE === "matching-headings"){
            const { ANSWER_SETTINGS, QUESTION, QUESTION_SETTINGS } = IELTS_LMS_QUESTION_MATCHING_HEADINGS_READING;
            ...
            const { ANSWER_SETTINGS, QUESTION, QUESTION_SETTINGS } = IELTS_LMS_QUESTION_MATCHING_INFORMATION_READING;
            const answerType = ANSWER_SETTINGS.ANSWER_TYPE;
            const answerOptions = ANSWER_SETTINGS.ANSWER_TYPES[answerType] || [];
            const numberOfQuestions = QUESTION_SETTINGS.NUMBER_OF_QUESTIONS;
            const numberOfOptions = ANSWER_SETTINGS.NUMBER_OF_OPTIONS || 1;
            const questionType = "type_select";  
            const updatedHtml           = replaceCurlyBracketsWithSelect(QUESTION.HTML, answerOptions, numberOfQuestions, numberOfOptions);
            const updateHtmlWithResult  = createExplanations(QUESTION.HTML, answerOptions, numberOfQuestions, numberOfOptions);
            setTotal(numberOfQuestions);
            ...
            const { ANSWER_SETTINGS, QUESTION, QUESTION_SETTINGS } = IELTS_LMS_QUESTION_TRUE_FALSE_NOTGIVEN_READING;
            const answerType            = ANSWER_SETTINGS.ANSWER_TYPE;
            const answerOptions         = ANSWER_SETTINGS.ANSWER_TYPES[answerType] || [];
            const numberOfQuestions     = QUESTION_SETTINGS.NUMBER_OF_QUESTIONS;
            const numberOfOptions       = ANSWER_SETTINGS.NUMBER_OF_OPTIONS || 1;
            ...
            const { ANSWER_SETTINGS, QUESTION, QUESTION_SETTINGS } = IELTS_LMS_QUESTION_YES_NO_NOTGIVEN_READING;
            const answerType            = ANSWER_SETTINGS.ANSWER_TYPE;
            const answerOptions         = ANSWER_SETTINGS.ANSWER_TYPES[answerType] || [];
            const numberOfQuestions     = QUESTION_SETTINGS.NUMBER_OF_QUESTIONS;
            const numberOfOptions       = ANSWER_SETTINGS.NUMBER_OF_OPTIONS || 1;
            ...
            const { QUESTION, QUESTIONS, QUESTION_SETTINGS, EXPLANATIONS } = IELTS_LMS_QUESTION_SUMMARY_FORM_COMPLETION_READING;
            const numberOfQuestions      = QUESTION_SETTINGS.NUMBER_OF_QUESTIONS;
            ...
            const { QUESTION, QUESTIONS, QUESTION_SETTINGS, EXPLANATIONS, ANSWER_SETTINGS } = IELTS_LMS_QUESTION_SENTENCE_COMPLETION_READING;
            const numberOfQuestions         = QUESTION_SETTINGS.NUMBER_OF_QUESTIONS;
            const answerType                = ANSWER_SETTINGS.ANSWER_TYPE;
            const answerOptions             = ANSWER_SETTINGS.ANSWER_TYPES[answerType] || [];
            const numberOfOptions           = ANSWER_SETTINGS.NUMBER_OF_OPTIONS || 1;
            ...
            const { QUESTION, QUESTION_SETTINGS, EXPLANATIONS, ANSWER_SETTINGS } = IELTS_LMS_QUESTION_PLAN_MAP_DIAGRAM_LABELLING_READING;
            const numberOfQuestions         = QUESTION_SETTINGS.NUMBER_OF_QUESTIONS;
...
const VerifyQuestion = () => {
    ...
    const validateQuestion = () => {
        ...
        if(IELTS_LMS_QUESTION_TYPE !== "none"){
            ...
            if(IELTS_LMS_QUESTION_TYPE === "matching-headings"){
                create = true;
                question = IELTS_LMS_QUESTION_MATCHING_HEADINGS_READING;
            } else if(IELTS_LMS_QUESTION_TYPE === "matching-information"){
                create = true;
                question = IELTS_LMS_QUESTION_MATCHING_INFORMATION_READING;
            } else if(IELTS_LMS_QUESTION_TYPE === "true-false-notgiven"){
                create = true;
                question = IELTS_LMS_QUESTION_TRUE_FALSE_NOTGIVEN_READING;
            } else if(IELTS_LMS_QUESTION_TYPE === "yes-no-notgiven"){
                create = true;
                question = IELTS_LMS_QUESTION_YES_NO_NOTGIVEN_READING;
            } else if(IELTS_LMS_QUESTION_TYPE === "summary-form-completion"){
                create = true;
                question = IELTS_LMS_QUESTION_SUMMARY_FORM_COMPLETION_READING;
            } else if(IELTS_LMS_QUESTION_TYPE === "sentence-completion"){
                create = true;
                question = IELTS_LMS_QUESTION_SENTENCE_COMPLETION_READING;
            } else if(IELTS_LMS_QUESTION_TYPE === "plan-map-diagram-labelling"){
                create = true;
                question = IELTS_LMS_QUESTION_PLAN_MAP_DIAGRAM_LABELLING_READING;
            }
        }
    ...
    useEffect(() => {
        if(IELTS_LMS_QUESTION_TYPE === "matching-headings"){
            setTitle(IELTS_LMS_QUESTION_MATCHING_HEADINGS_READING.TITLE);
            setOrder(IELTS_LMS_QUESTION_MATCHING_HEADINGS_READING.order);
        } else  if(IELTS_LMS_QUESTION_TYPE === "matching-information"){
            setTitle(IELTS_LMS_QUESTION_MATCHING_INFORMATION_READING.TITLE);
            setOrder(IELTS_LMS_QUESTION_MATCHING_INFORMATION_READING.order);
        } else  if(IELTS_LMS_QUESTION_TYPE === "true-false-notgiven"){
            setTitle(IELTS_LMS_QUESTION_TRUE_FALSE_NOTGIVEN_READING.TITLE);
            setOrder(IELTS_LMS_QUESTION_TRUE_FALSE_NOTGIVEN_READING.order);
        } else if(IELTS_LMS_QUESTION_TYPE === "yes-no-notgiven"){
            setTitle(IELTS_LMS_QUESTION_YES_NO_NOTGIVEN_READING.TITLE);
            setOrder(IELTS_LMS_QUESTION_YES_NO_NOTGIVEN_READING.order);
        } else if(IELTS_LMS_QUESTION_TYPE === "summary-form-completion"){
            setTitle(IELTS_LMS_QUESTION_SUMMARY_FORM_COMPLETION_READING.TITLE);
            setOrder(IELTS_LMS_QUESTION_SUMMARY_FORM_COMPLETION_READING.order);
        } else if(IELTS_LMS_QUESTION_TYPE === "sentence-completion"){
            setTitle(IELTS_LMS_QUESTION_SENTENCE_COMPLETION_READING.TITLE);
            setOrder(IELTS_LMS_QUESTION_SENTENCE_COMPLETION_READING.order);
        } else if(IELTS_LMS_QUESTION_TYPE === "plan-map-diagram-labelling"){
            setTitle(IELTS_LMS_QUESTION_PLAN_MAP_DIAGRAM_LABELLING_READING.TITLE);
            setOrder(IELTS_LMS_QUESTION_PLAN_MAP_DIAGRAM_LABELLING_READING.order);
        }
...
const EditQuestionModal = ({questionId,editQuestionModal,handleCloseEditQuestion}) => { 
    ...
    const loadData = (data = {}) => {
        if(data.TYPE === "matching-headings"){
            dispatch(update_ielts_lms({
                IELTS_LMS_QUESTION_MATCHING_HEADINGS_READING: data,
                IELTS_LMS_QUESTION_TYPE: 'matching-headings',
                IELTS_LMS_QUESTION_MODE: 'edit',
                IELTS_CURRENT_QUESTION_ID: questionId
            }));
            setLoading(false);
        } else if(data.TYPE === "matching-information"){
            dispatch(update_ielts_lms({
                IELTS_LMS_QUESTION_MATCHING_INFORMATION_READING: data,
                IELTS_LMS_QUESTION_TYPE: 'matching-information',
                IELTS_LMS_QUESTION_MODE: 'edit',
                IELTS_CURRENT_QUESTION_ID: questionId
            }));
            setLoading(false);
        } else if(data.TYPE === "true-false-notgiven"){
            dispatch(update_ielts_lms({
                IELTS_LMS_QUESTION_TRUE_FALSE_NOTGIVEN_READING: data,
                IELTS_LMS_QUESTION_TYPE: 'true-false-notgiven',
                IELTS_LMS_QUESTION_MODE: 'edit',
                IELTS_CURRENT_QUESTION_ID: questionId
            }));
            setLoading(false);
        } else if(data.TYPE === "yes-no-notgiven"){
            dispatch(update_ielts_lms({
                IELTS_LMS_QUESTION_YES_NO_NOTGIVEN_READING: data,
                IELTS_LMS_QUESTION_TYPE: 'yes-no-notgiven',
                IELTS_LMS_QUESTION_MODE: 'edit',
                IELTS_CURRENT_QUESTION_ID: questionId
            }));
            ...
            dispatch(update_ielts_lms({
                IELTS_LMS_QUESTION_SUMMARY_FORM_COMPLETION_READING: data,
                IELTS_LMS_QUESTION_TYPE: 'summary-form-completion',
                IELTS_LMS_QUESTION_MODE: 'edit',
                IELTS_CURRENT_QUESTION_ID: questionId
            }));
            ...
            dispatch(update_ielts_lms({
                IELTS_LMS_QUESTION_SENTENCE_COMPLETION_READING: data,
                IELTS_LMS_QUESTION_TYPE: 'sentence-completion',
                IELTS_LMS_QUESTION_MODE: 'edit',
                IELTS_CURRENT_QUESTION_ID: questionId
            }));
            ...
            dispatch(update_ielts_lms({
                IELTS_LMS_QUESTION_PLAN_MAP_DIAGRAM_LABELLING_READING: data,
                IELTS_LMS_QUESTION_TYPE: 'plan-map-diagram-labelling',
                IELTS_LMS_QUESTION_MODE: 'edit',
                IELTS_CURRENT_QUESTION_ID: questionId
            }));
...
export { IeltsLmsEditQuizReadingPassages, IeltsLmsEditQuizReadingQuestions };
